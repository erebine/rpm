...
[releases](https://github.com/erebine/rpm/releases). Each one is also
...
``` shell
curl -fLO https://github.com/erebine/rpm/releases/latest/download/erectl.x86_64.rpm
sudo dnf install -y ./erectl.x86_64.rpm
```

The architecture stays in that name -- `erectl.aarch64.rpm` is the 64-bit
ARM package, built by the same workflow -- so the unversioned filename can
never resolve to a package built for another machine. The versioned filename
(`erectl-2.0.0-1.x86_64.rpm`) is attached to the same release for pinning to
a specific version.
